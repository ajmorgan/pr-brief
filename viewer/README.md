...
The editor lives inside the pr-brief skill; this is its only copy. After changing app code or CSS run `node build.mjs --stamp` (re-stamps the service worker and import map; needs no dependencies). Only a change to `src/vendor-entry.js`, `src/mermaid-entry.js` or the npm dependencies needs the full `npm install && npm run build`, which requires `node_modules`.


```sh
npm install        # once
npm run build      # bundles CodeMirror into vendor/editor.js, mermaid into vendor/mermaid.js, and stamps sw.js
...
- **Mermaid diagrams**: a ```` ```mermaid ```` fence is drawn as inline SVG in the current color scheme (the scheme's colors feed mermaid's theme, and a scheme change redraws). The 5 MB mermaid bundle loads on the first diagram only. Diagrams run at mermaid's strict security level and pass through DOMPurify; a fence mermaid cannot parse shows the error above its source. See `src/lib/mermaid.js`.
...
src/lib/                themes, settings (localStorage), store (IndexedDB),
                        markdown (marked + DOMPurify + Lezer highlighting), mermaid
                        (```mermaid fences → SVG, lazily), files, brief (pr-brief model)
src/vendor-entry.js     the npm imports; bundled by esbuild into vendor/editor.js
src/mermaid-entry.js    mermaid alone; bundled into vendor/mermaid.js, imported on demand
```

Only the third-party libraries are bundled. The application itself is plain ES modules loaded by the browser, built on the web platform directly: custom elements with light DOM, plain module state and DOM events instead of a framework (`src/lib/state.js` holds only `debounce`, `isMac` and `modKey`), `<dialog>` for modals, IndexedDB for records, localStorage for tiny preferences, and the File System Access API for user-owned documents.
