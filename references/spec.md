...
**Method Context:** <concise summary, past tense — SLOT>

**Changes:** <"Deleted; …" — what the removal takes away and what replaces it — SLOT>
...
- `new` units have `Context:` only. `deleted` and `modified` units have both `Context:` and `Changes:`; a deleted unit's `Context:` is in the past tense and its `Changes:` starts with the word Deleted, so the deletion is stated rather than left to be inferred from a missing label.
...
- `<Kind> Context:` (past tense, deleted unit) — concise summary. What the deleted function used to do.
- `Changes:` (deleted unit) — one concise line starting with the word Deleted: what the removal takes away, and what replaces it when the agent can see it; when the Callers line lists sites, that they are now broken unless they were changed too. Was absent until 2026-09-09: a missing label read as "nothing to say", not as "deleted".
...
`SKILL.md` instructs the agent to: run extract (stop and relay verbatim on exit 2 — §4a) → read skeleton and the changed files → fill every file section (files in order, units in order) → write `Overview:` last → run lint → repeat until clean → open the brief in the browser (`viewer.ts --out`, §18.1) and print the path and the top summary block. The agent never generates headings, dividers, or hunks itself.
...
`extract --open` starts `scripts/viewer.ts`, a zero-dependency localhost server on the fixed port 8790, and opens `http://127.0.0.1:8790/?brief=/briefs/<slug>`. One server per machine: a second start hands its brief (and repository root) to the running one through `/switch` and exits; the running server adds it to the set it serves and makes it current, and the open tab lists it and follows — and when no tab is connected (`/switch` answers with `tabs`, the count of open `/events` streams) the second start opens a browser on it, so the brief comes up whether or not the reader closed the last tab. SKILL.md's last step runs `viewer.ts --detach --out <path>` on the finished brief for that reason, whatever flags extract was given; `--detach` starts the server as a background process when none is running, so the agent's shell returns at once. Several briefs are served at once: every brief under a repository's state directory (`<common git dir>/pr-brief/<key>/pr-brief-<key>.md`, §9a) joins the set at `/briefs/<key>`, and each reads its files from the worktree recorded in its front matter. `--verbose` logs requests; `--stop` asks the running server to exit. The server serves `viewer/` statically (no-store) and exposes:
...
| `POST /switch` | `{ path, root }` — add a brief (or find it) and make it current; token required; answers `{ ok, path, slug, url, tabs }`, `tabs` being the open tabs that will follow |
